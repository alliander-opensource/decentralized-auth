// SOURCE: https://github.com/iotaledger/kerl/blob/master/javascript/kerl.js
...
Kerl.prototype.initialize = function (state) {
  ...
  for (let i = 0; i < (this.HASH_LENGTH * 3); i++) {
...
Kerl.prototype.absorb = function (trits, offset, length) {
  ...
  do {
    let i = 0;
    const limit = (length < this.HASH_LENGTH ? length : this.HASH_LENGTH);
    ...
    const bytesToAbsorb = Converter.bytesFromBigInt(Converter.bigIntFromTrits(this.trit_state, this.HASH_LENGTH), this.BYTE_HASH_LENGTH);
