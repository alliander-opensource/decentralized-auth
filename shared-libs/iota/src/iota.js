...
module.exports = class IotaClient { // eslint-disable-line padded-blocks
  ...
  send(seed, receiver, message) {
    return new Promise((resolve, reject) => {
      ...
      this.iota.api.sendTransfer(seed, this.depth, this.mwm, transfers, (err, res) => {
        if (!err) {
          if (this.logger) this.logger.info(`Send message ${JSON.stringify(message)} to ${receiver}`);
          return resolve(res);
        }
        if (this.logger) this.logger.error(`Send error: ${err}`);
        return reject(err);
      });
    });
  }
